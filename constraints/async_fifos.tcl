...
foreach fifo $async_fifos {
	...
	if {!($reset_ffs eq "")} {
		...
		if {!([get_pins $fifo/input_rst_sync2_reg_reg/D] eq "")} {
			set_false_path -to [get_pins $fifo/input_rst_sync2_reg_reg/D]
		}
		if {!([get_pins $fifo/output_rst_sync2_reg_reg/D] eq "")} {
	...
	if {[string match [get_property ORIG_REF_NAME [get_cells $fifo]] axis_async_frame_fifo] || [string match [get_property REF_NAME [get_cells $fifo]] axis_async_frame_fifo]} {
		set status_sync_regs [get_cells -quiet -hier -regexp {.*/(?:overflow|bad_frame|good_frame)_sync[234]_reg_reg} -filter "PARENT == $fifo"]
		foreach reg $status_sync_regs {
			set_property ASYNC_REG TRUE $reg
		}
		set sync2 [get_cells -quiet -hier -regexp {.*/(?:overflow|bad_frame|good_frame)_sync2_reg_reg} -filter "PARENT == $fifo"]
		foreach reg $sync2 {
...
set eth_mac_1g_fifo_inst [get_cells -hier -filter {ORIG_REF_NAME == eth_mac_1g_fifo || REF_NAME == eth_mac_1g_fifo}]
if { !($eth_mac_1g_fifo_inst eq "") } {
	...
	set rx_clk [get_clocks -of_objects [get_cells $eth_mac_1g_fifo_inst/rx_sync_reg_1_reg*]]
	set_max_delay -from [get_cells -hier -regexp {.*/rx_sync_reg_1_reg\[[012]\]} -filter "PARENT == $eth_mac_1g_fifo_inst"] -to [get_cells -hier -regexp {.*/rx_sync_reg_2_reg\[[012]\]} -filter "PARENT == $eth_mac_1g_fifo_inst"] -datapath_only [get_property -min PERIOD $rx_clk]
}
